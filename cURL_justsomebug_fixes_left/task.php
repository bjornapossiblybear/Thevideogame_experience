...
//update a task.
if ($_SERVER['REQUEST_METHOD'] == "PUT") { #our api only responds to PUT request.
...
/* need to put the other fields above */
if (!$dbconnecterror) {
   try {
   ...
   try {
	...
	if (!$dbconnecterror) {
		try {
			...
			$Tasklist = [
			"listID" => $anIdcode , "complete" => $complete ,  "listItem" => $taskName , "finishDate" => $taskDate ]; 
			http_response_code(204);
			exit();
		} catch (PDOException $e) {
		/* return 500 message */
